import {RESTART_GAME, MAKE_GUESS, GENERATE_AURAL_UPDATE} from './actions';

const initialState = {
  guesses: [],
  feedback: 'Make your guess!',
  auralStatus: '',
  correctAnswer: Math.round(Math.random() * 100) + 1
};

export default (state = initialState, action) => {
  if (action.type === MAKE_GUESS){
    let feedback, guess;

    guess = parseInt(action.guess, 10);
    if(isNaN(guess)){
      feedback = 'Please enter a valid number.';
      // merge guesses into state
      return Object.assign({}, state, {
        feedback,
        guesses: [...state.guesses, guess]
      });
    }
    // get difference to determine how far from correct answer
    // from game.js
    const difference = Math.abs(guess - state.correctAnswer);

    if(difference >= 50){
      feedback = "You're Ice C;old.";
    } else if (difference >= 30){
      feedback = "You're Cold.";
    } else if (difference >= 10){
      feedback = "You're Warm.";
    } else if (difference >= 1){
      feedback = "You're Hot!";
    } else {
      feedback = 'You got it!';
    }
    // merge guesses into state
    return Object.assign({}, state, {
      feedback,
      guesses: [...state.guesses, guess]
    })
  }

  if(action.type === RESTART_GAME){
    return Object.assign({}, state, {
      guesses: [],
      feedback: 'Make a guess!',
      auralStatus: '',
      correctAnswer: action.correctAnswer
    })
  }

  if(action.type === GENERATE_AURAL_UPDATE){
    const {guesses, feedback} = state;

    // if more than 1 guess, pluralize nouns in aural update
    const pluralize = guesses.length !== 1;

    let auralStatus = `Current status of the game: ${feedback} You've made ${guesses.length} ${pluralize ? 'guesses' : 'guess'}.`;

    if(guesses.length > 0){
      auralStatus += `${pluralize
        ? 'In order of most- to least-recent, they are'
        : 'It was'}: ${guesses.reverse().join(', ')
      }`;
    }

    return Object.assign({}, state, {auralStatus});
  }

  return state;
}